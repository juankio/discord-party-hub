<template>
  <div 
    ref="tokenContainer"
    class="absolute z-10 w-[60px] h-[60px] -ml-[30px] -mt-[30px] flex items-center justify-center parchis-token transition-all duration-300 pointer-events-auto cursor-pointer"
    :style="{
  ...
  >
    <div class="token-body relative w-3/4 h-3/4 flex items-center justify-center transform-style-3d"
       ...
       <div class="w-full h-full rounded-full border-4" 
            :style="{ 
              backgroundColor: token.color, 
              borderColor: 'rgba(0,0,0,0.3)',
              background: `radial-gradient(circle at 30% 30%, rgba(255,255,255,0.7), ${token.color} 50%, rgba(0,0,0,0.4))`
            }">
          <div class="absolute inset-2 rounded-full shadow-[0_4px_4px_rgba(0,0,0,0.4)]"
               :style="{ 
