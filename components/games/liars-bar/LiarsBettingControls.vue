  ...
  <div class="flex items-center justify-center gap-3 md:gap-5 w-full">
    ...
    <div class="flex items-center bg-[#1f120a] rounded-md p-1.5 border-2 border-[#150c07] shadow-[inset_0_4px_8px_rgba(0,0,0,0.8)]">
      <button 
        ...
        :disabled="displayFace <= 1"
        :class="{ 'opacity-50 cursor-not-allowed hover:bg-[#4a2e1b]': displayFace <= 1 }"
      ...
      <div class="w-14 md:w-16 h-12 flex justify-center items-center">
        ...
        <div class="w-8 h-8 md:w-10 md:h-10 bg-[#e8e4d9] rounded shadow-[1px_3px_0px_#8a7f6c,inset_-1px_-2px_4px_rgba(0,0,0,0.3)] flex items-center justify-center relative border border-[#d3cebe]">
          <div v-if="displayFace === 1" class="w-2 h-2 bg-[#2c2620] rounded-full shadow-inner"/>
          <div v-else-if="displayFace === 2" class="w-full h-full p-2 flex flex-col justify-between items-center">
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full self-end shadow-inner"/>
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full self-start shadow-inner"/>
          </div>
          <div v-else-if="displayFace === 3" class="w-full h-full p-1.5 flex flex-col justify-between items-center">
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full self-end shadow-inner"/>
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full shadow-inner"/>
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full self-start shadow-inner"/>
          </div>
          <div v-else-if="displayFace === 4" class="w-full h-full p-2 grid grid-cols-2 gap-1 place-items-center">
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full shadow-inner"/>
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full shadow-inner"/>
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full shadow-inner"/>
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full shadow-inner"/>
          </div>
          <div v-else-if="displayFace === 5" class="w-full h-full p-1.5 grid grid-cols-2 gap-1 place-items-center relative">
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full shadow-inner"/>
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full shadow-inner"/>
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 shadow-inner"/>
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full shadow-inner"/>
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full shadow-inner"/>
          </div>
          <div v-else-if="displayFace === 6" class="w-full h-full p-1.5 grid grid-cols-2 gap-0.5 place-items-center">
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full shadow-inner"/>
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full shadow-inner"/>
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full shadow-inner"/>
            <div class="w-1.5 h-1.5 bg-[#2c2620] rounded-full shadow-inner"/>
      ...
      <button 
        class="w-11 h-11 md:w-12 md:h-12 bg-[#4a2e1b] hover:bg-[#5c3a21] active:bg-[#382214] border-b-[5px] border-[#25160d] active:border-b-0 active:translate-y-[5px] rounded text-xl font-bold flex justify-center items-center transition-all select-none"
        :disabled="displayFace >= 6"
        :class="{ 'opacity-50 cursor-not-allowed hover:bg-[#4a2e1b]': displayFace >= 6 }"
...
const displayAmount = computed(() => {
  if (typeof props.amount !== 'number' || Number.isNaN(props.amount)) return 1;
  return props.amount;
});

const displayFace = computed(() => {
  if (typeof props.face !== 'number' || Number.isNaN(props.face)) return 2;
  return Math.min(6, Math.max(1, props.face));
});
