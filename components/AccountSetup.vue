<template>
  <div class="flex flex-col items-center gap-4 w-full">
    ...
    <div v-else class="w-full flex flex-col gap-6 mt-4">
      ...
      <button @click="loginWithGoogle" class="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-200 text-black px-6 py-4 rounded-full font-black transition-all hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.2)]">
      ...
      <p class="text-[10px] text-gray-600 text-center uppercase tracking-widest mt-4">La API Key de Google se conectará pronto</p>
    </div>
  </div>
</template>
...
const loginWithGoogle = async () => {
  errorMsg.value = 'Configurando OAuth de Google... (A la espera de la API Key)'
  // Aquí usaremos la API key que proporcionará el usuario.
  // Por ahora simularemos la redirección o el pop-up de Google.
  /*
  const res = await $fetch('/api/auth/google', {
    method: 'POST',
    body: { token: googleCredentialToken }
  })
  if (res.token) playerStore.setAccountAuth(res.token, res.user)
  */
}
