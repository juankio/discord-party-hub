<template>
  <div
    ...
    :class="[
      ...
      { 'cursor-pointer hover:z-50': isBot && isHost && !player.isEmpty }
    ]"
    :style="{
      left: position.left,
      top: position.top,
      transform: isHovered
        ...
        : position.transform,
      borderColor: player.isEmpty ? (isHovered ? localPlayerColorSafe : '') : playerColor,
      backgroundColor: player.isEmpty && isHovered ? 'rgba(0,0,0,0.6)' : '',
      boxShadow: player.isEmpty
  ...
  >
    ...
    <img v-if="!player.isEmpty" :src="`/avatars/avatar-${player.avatarId}.svg?v=2`" class="w-full h-full rounded-full object-contain bg-[#151515]" :class="{'grayscale opacity-50': player.isOffline}">

    <!-- Elementos externos al avatar -->
    <div v-if="isHostPlayer && !player.isEmpty" class="absolute -top-4 -right-2 z-20 rotate-12 drop-shadow-md">
